...
def create_pptx_from_structure(data):
    ...
    # Intenta usar el layout blank (6), si falla usa el primero (0)
    try:
    ...
    # Detectamos qué tipo de información trajo la IA para elegir el diseño
    ...
    if "dofa" in t_type or "swot" in t_type or ("fortalezas" in keys and "amenazas" in keys):
        ...
        # que imprime TODO lo que encuentre como una lista limpia.
...
def _draw_dofa_layout(slide, data):
    ...
    # Coordenadas base para los 4 cuadrantes
    margin_x = 0.5
    margin_y = 1.8
    w = 6.0
    h = 2.2
    ...
    # Mapeo de cuadrantes: Título, Datos, Posición X, Posición Y
    quadrants = [
    ...
    for title, points, x, y in quadrants:
        ...
        for point in points_list[:5]: # Limitar a 5 bullets para que quepa bien
            p = tf.add_paragraph()
            p.text = f"• {point}"
            p.font.size = Pt(11)

def _draw_journey_table(slide, data):
    """Detecta etapas y crea una tabla visual nativa."""
...
def _draw_persona_layout(slide, data):
    """Diseño con barra lateral para perfil y dos columnas para necesidades/dolores."""
...
def _draw_matrix_layout(slide, data):
    """Diseño para Matriz de Posicionamiento (2x2)"""
    # Por ahora usamos el genérico inteligente para la matriz, 
    # pero se puede expandir en el futuro con un gráfico de dispersión.
    _draw_smart_generic_layout(slide, data) 

def _draw_smart_generic_layout(slide, data):
    """
    Dibuja cualquier otra estructura como una lista limpia.
    Evita que salgan slides vacíos si el template no es uno de los conocidos.
    """
    ...
    # Caja de texto grande para el contenido
...
def _draw_footer_insight(slide, text):
    # Caja destacada al final para el insight principal
    ...
    # Ajuste fino de márgenes para centrar verticalmente
    tf.margin_top = Inches(0.2)
