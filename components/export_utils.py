...
def render_chat_history(history, source_mode="chat"):
    ...
    for msg in history:
        role = msg["role"]
        content = msg["content"] # Contenido íntegro original con metadatos
        avatar = "✨" if role == "assistant" else "👤"
        
        with st.chat_message(role, avatar=avatar):
            if role == "assistant":
                # LIMPIEZA SOLO PARA LA PANTALLA:
                # 1. Cortar si detecta el bloque técnico con las barras |||
                display_text = re.split(r'\[\d+\].*?\|\|\|', content, flags=re.DOTALL)[0]
                # 2. Cortar si detecta la sección de "Fuentes" escrita al final
